...
This one is loading the 3rd version of the data including the status variable and holding setpoint and state/status data in discrete form in the data.
...
(Note, we've manually removed the funny last datapoint that showed the date of extraction for pretty much each dataset)

```{r load-single, eval=FALSE}

jan <- readxl::read_xlsx(paste0(root_path, "/data/v3/input/2021-jun.xlsx"))
...
name_vector <- names(jan)
all(names(jan) == name_vector)

# Reshape.
(jan_orig <- jan %>% 
  select(time = 1, state = 2, status = 3, setpoint = 4, temp = 5))
...
p1 <- ggplot(data = jan_orig, aes(x = time)) +
  geom_line(aes(y = temp, colour = "temp")) +
  geom_line(aes(y = setpoint, colour = "setpoint")) +
  scale_colour_manual("", breaks = c("temp", "status", "state", "setpoint"), values = c("grey", "orange", "pink", "lightskyblue"))

p2 <- ggplot(data = jan_orig, aes(x = time)) +
...
Looking good. Let's get them all in...
...
files <- list.files(path = paste0(root_path, "/data/v3/input"))

files_clean <- tibble(file_name = files) %>% 
  filter(!str_starts(file_name, '~')) %>% # remove open books
  pull(file_name)

file_paths <- sapply(files_clean, function(file) { paste0(root_path, "/data/v3/input/", file)  }, USE.NAMES = FALSE)
...
```
We only have as many cases as there are events. So above seems to make sense considering modt work happends in Q3.
...
df_orig %>% filter(is.na(temp), is.na(status), is.na(state), is.na(setpoint))

# Remove appropriately.
(df <- df_orig %>% 
  filter(!is.na(temp) | !is.na(status) | !is.na(state) | !is.na(setpoint)))
...
# Check.
df %>% 
  group_by(time, state, status, setpoint, temp) %>% 
  count() %>% 
  filter(n > 1)

# Do.
(df <- df %>% 
  distinct(time, state, status, setpoint, temp, .keep_all = TRUE))
...
write_csv(df, paste0(root_path, '/data/v3/df-clean.csv', na = ""))
