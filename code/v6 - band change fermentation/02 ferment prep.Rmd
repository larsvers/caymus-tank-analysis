...
C1 showed an example of an incorrect fermentation period that lingered at around 20 brix before falling off, registering as a false positive fermentation.

The following procedure defends against that case by:

1. Checking if the 10 valued rolling brix before the drop off was above 10 
2. It should be around 0 or below, so if it's high it's defined as a false positive and the whole period will be removed
...
# Add gradient.
df_test <- df_test %>% 
  mutate(brix_gradient = pracma::gradient(df_test$brix_smooth))
...
p2 <- df_test %>% 
  ggplot(aes(x = time)) +
    geom_line(aes(y = brix_gradient), colour = "blue")
...
So after trying every above combination I think we should stick to simple and take the first row with brix_gradient < 0 as point(2)
...
For each fermentation period calculate the LOESS smooth curve and the smooth curve's gradient. We'll use the smooth brix' gradient to detect points 2 (curve start), 3 (curve inflection) and 4 (curve end).
...
Note we're using r's base loess function with a span we set to 0.25 (which seems most plausible). We're NOT using STL decomposition with a loess trend as this is just much simpler :))
...
df_brix_smooth <- df_brix %>% 
  ...
  # Get brix_smooth and brix_gradient
  # ---------------------------------
  ...
  mutate(
    m = purrr::map(data, loess, formula = brix ~ index, span = 0.25), # we take 0.25 as it fits nicely so far
    brix_smooth = purrr::map(m, `[[`, "fitted"),
    brix_gradient = purrr::map(brix_smooth, pracma::gradient)
  ) %>% 
  ...
  # Unnest the data :)
  tidyr::unnest(cols = c(data, brix_smooth, brix_gradient)) %>% 

  # Get brix_gradient_smooth
  # ------------------------

  # Nest by ferment_period and get the gradieng in
  tidyr::nest(data = c(-ferment_period, index, time, brix_smooth, brix_gradient)) %>% 

  # for each ferment_period calculate the loess for their fitted values
  # and the gradient for the resulting brix_smooth values.
  mutate(
    m = purrr::map(data, loess, formula = brix_gradient ~ index, span = 0.25), # trying 0.25 again
    brix_gradient_smooth = purrr::map(m, `[[`, "fitted")
  ) %>% 
  ...
  # Unnest the data :)
  tidyr::unnest(cols = c(data, brix_gradient_smooth)) %>% 

  # Clean up
  select(-index) %>% 
  select(time, brix, brix_smooth, brix_gradient, brix_gradient_smooth, ferment, ferment_bool, ferment_period, ferment_point, ferment_phase) %>% 
  arrange(time)
...
# Check brix_smooth and brix_gradient make sense.
df_brix_smooth %>% 
  filter(ferment_bool) %>% 
  ggplot(aes(x = time)) +
    geom_line(aes(y = brix)) +
...
df_brix_smooth %>%
  filter(ferment_bool) %>% 
  ggplot(aes(x = time)) +
    geom_line(aes(y = brix_gradient_smooth), colour = 'steelblue') +
    facet_wrap(~ferment_period, scales = "free", nrow = 2)
...
ferment_curve_start_times <- df_brix_smooth %>% 
  filter(ferment_bool, brix_gradient < 0) %>% 
  group_by(ferment_period) %>% 
  mutate(first = first(brix_gradient)) %>% 
  ungroup() %>% 
  select(time, first) %>% 
  distinct(first, .keep_all = TRUE) %>% 
  pull(time)

# Get the times for point (4)
...
inflection_times <- df_brix_smooth %>% 
  ...
  slice_min(brix_gradient_smooth) %>% 
  ungroup() %>% 
  pull(time)
...
Check for all points over brix visually:

```{r}

# Check ferment start (point 1), end (point 5) and the newly created points 2 and 4 are detected correctly.
...
df %>% 
  ...
  fill(brix_smooth, brix_gradient, brix_gradient_smooth, ferment_bool) %>% 
